...
public class AdminInventariosController {
    ...
    public boolean actualizarInventario(Inventarios inventario) {
        ...
        if (inventario.getstockActual() < 0 || inventario.getstockMinimo() < 0 || inventario.getstockReservado() < 0) {
            return false;
        }

        Inventarios actual = inventariosDAO.consultarInventarioPorId(inventario.getidInventario());
        if (actual == null) {
            return false;
        }

        inventario.setproductoId(actual.getproductoId());
        inventario.setfechaCreacion(actual.getfechaCreacion());
        inventario.setfechaActualizacion(new Timestamp(System.currentTimeMillis()));
